...
CV.simulation = function(DR, DO, alpha, k0, E0, C, voltage.range, v){
  ...
  for(loop in 1:3){
    for(i1 in 1:L){ # Time index
      ...
      for(i2 in 2:(j-1)){ # Space index
        O[i1+1, i2] = O[i1, i2] + DMo*(O[i1, i2+1] + O[i1, i2-1] - 2*O[i1, i2]) # Discretized diffusion
      ...
      O[i1+1, 1] = O[i1+1, 2] - J0[i1+1]*(DOx[i1]/DO)
...
test = rbind(CV.simulation(DR = 1e-5, DO = 1e-5, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.001), 
             CV.simulation(DR = 1e-5, DO = 1e-5, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01))
ggplot(test) +
...
test = CV.simulation(DR = 1e-5, DO = 1e-5, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01)
test$DO = 1e-5
tes2 = CV.simulation(DR = 1e-5, DO = 1e-7, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01)
tes2$DO = 1e-7
test = rbind(test, tes2); rm(tes2)
tes2 = CV.simulation(DR = 1e-5, DO = 1e-3, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01)
tes2$DO = 1e-3
test = rbind(test, tes2); rm(tes2)

g1 = ggplot(test) +
  geom_path(mapping = aes(x = voltage, y = current, color = as.factor(log10(DO)))) +
  labs(x = "Potential vs ref", y = "Current density (mA/cm2)", color = expression("log10 D"[o]), subtitle = expression("log10 D"[r]*" = -5")) + theme(legend.position = c(0.1, 0.85))
# g1
test = CV.simulation(DR = 1e-5, DO = 1e-5, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01)
test$DR = 1e-5
tes2 = CV.simulation(DR = 1e-7, DO = 1e-5, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01)
tes2$DR = 1e-7
test = rbind(test, tes2); rm(tes2)
tes2 = CV.simulation(DR = 1e-3, DO = 1e-5, alpha = 0.35, k0 = 0.01, E0 = 0.35, C = 1, 
                           voltage.range = c(seq(from = 0, to = 0.7, by = 0.01), seq(from = 0.7, to = 0, by = -0.01)), v = 0.01)
tes2$DR = 1e-3
test = rbind(test, tes2); rm(tes2)

g2 = ggplot(test) +
  geom_path(mapping = aes(x = voltage, y = current, color = as.factor(log10(DR)))) +
  labs(x = "Potential vs ref", y = "Current density (mA/cm2)", color = expression("log10 D"[r]), subtitle = expression("log10 D"[o]*" = -5")) + theme(legend.position = c(0.1, 0.85))
(g1 + theme(legend.position = c(0.1, 0.5)) + labs(title = "Constant step size")) / g2 + theme(legend.position = c(0.1, 0.5))
...
CV.fit.function = function(voltage, scanrate, logDR, logDO, alpha, logk0, E0, C){
  ...
  DR = 10^logDR; DO = 10^logDO; k0 = 10^logk0
  ...
  for(v in unique(scanrate)){
    ...
    # Run the simulation
    CV.sim = CV.simulation(DR = DR, DO = DO, alpha = alpha, k0 = k0, E0 = E0, C = C, voltage.range = volt.range, v = v)
    # Combine and save
...
system.time({
  mod.GA <- ga(type = "real-valued",
               ...
               fitness =  function(var){-sum((CV.data.trim$current - (CV.fit.function(voltage = CV.data.trim$voltage, scanrate = CV.data.trim$scanrate, 
                                  ...
                                  logDR = var[1], logDO = var[2], alpha = var[3], logk0 = var[4], E0 = var[5], C = concentration)))^2*
                                  CV.data.trim$weight1*CV.data.trim$weight2)},
               # Bounds for each of the varaibles
               lower = c(-6, -6, 0.45, -1, 0.55), upper = c(-5, -5, 0.55, 1, 0.65), 
...
solution = data.frame(DR = 10^mod.GA@population[,1], DO = 10^mod.GA@population[,2], alpha = mod.GA@population[,3],
                      k0 = 10^mod.GA@population[,4], E0 = mod.GA@population[,5], fit = mod.GA@fitness) 
...
test.current = CV.fit.function(voltage = CV.data$voltage, scanrate = CV.data$scanrate,
                               ...
                               logk0 = mod.GA@solution[4], E0 = mod.GA@solution[5], C = concentration)
